...
namespace SbB.FEM
{
    class QuadraticFEMData: FEMData
    {
        public QuadraticFEMData(SubDomain subdomain) : base(subdomain)
        {
            ...
            if (Triangulation.Triangulator.triangulate())
            {
                ...
                {
                    triangles[i] = new QuadraticTriangle(triangles[i]);
                }
                ...
                for (int i = 0; i < boundaries.Count; i++)
                {
                    ...
                    for (int j = 0; j < boundaries[i].Count; j++)
                        boundaries[i][j] = new QuadraticEdge(boundaries[i][j]);
                }
            }
